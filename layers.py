...
def fully_connected(input, num_output, name):
	...
	with tf.variable_scope(name) as scope:
		weight = tf.get_variable("weight", shape = [input.shape.as_list()[1], num_output], initializer = xavier_init())
		return tf.matmul(input, weight)

def conv2d(input, filter_shape, strides = (1,1,1,1), activation = tf.nn.elu, pad = "SAME", name = None):
	with tf.variable_scope(name) as scope:
		filter = tf.get_variable("filter", shape = filter_shape, initializer = xavier_init_conv2d())
		conv_op = tf.nn.conv2d(input = input, filter = filter, strides = strides, padding = pad)
