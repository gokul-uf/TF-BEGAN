...
def get_noise_batch(size=[conf.batch_size, conf.embedding_dim]):
    return np.random.uniform(
        low=conf.z_low_limit, high=conf.z_high_limit, size=size)


def get_img_batch(img_ids,
                  loc=conf.data_location,
                  img_shape=(conf.img_height, conf.img_width)):
    assert len(img_ids) == conf.batch_size
    img_batch = np.zeros((conf.batch_size, conf.img_height, conf.img_width,
                          conf.num_channel))
    for i, img_id in enumerate(img_ids):
        img_file = "{}.jpg".format(str(img_id).zfill(
            6))  # CelebA images are numbered 000001, 000002...
    ...
    assert img_batch.shape == (conf.batch_size, conf.img_height,
                               conf.img_width, conf.num_channel)
    return img_batch
...
if __name__ == '__main__':
    ...
    input_image = tf.placeholder(
        tf.float32, [None, conf.img_height, conf.img_width, conf.num_channel],
        name="input_image")
    ...
    with tf.Session(config=config) as sess:
        ...
        for i in range(conf.num_epoch):
            ...
            for j in tqdm(
                    range(0,
                          conf.batch_size * (conf.dataset_size // conf.
                ...
                _, _, batch_d_loss, batch_g_loss, batch_l_x, batch_l_g_g = sess.run(
                    [d_train, g_train, l_d, l_g, l_x, l_g_g],
                    feed_dict=feed_dict)
            ...
            print("M_global: {}, G Loss: {}, D Loss: {}".format(
                M_global, g_loss, d_loss))
            k_t_input += conf.lambda_k * (conf.gamma * epoch_l_x - epoch_l_g_g)
            k_t_input = max(min(1, k_t_input),
                            0)  # to ensure 0 <= k_t_input <= 1

            if i % conf.sample_epoch == 0:
                print("Sampling Images")
                sample_noise = get_noise_batch(
                    size=[conf.num_samples, conf.embedding_dim])
                gen_images = sess.run(
                    gen_image_z_d, feed_dict={z_d: sample_noise})
                assert gen_images.shape == (conf.num_samples, conf.img_height,
                                            conf.img_width, conf.num_channel)
                gen_images = (gen_images * 127.) + 127.
                gen_images = [
                    Image.fromarray(gen_images[id].astype(np.uint8))
                    for id in range(conf.num_samples)
                ]  # Image expects uint8 [0, 255]
                sample_images = Image.new("RGB",
                                          (conf.num_cols * conf.img_width,
                                           conf.num_rows * conf.img_height))
                for row in range(conf.num_rows):
                    for col in range(conf.num_cols):
                        idx = row * conf.num_cols + col
                        sample_images.paste(gen_images[idx],
                                            (col * conf.img_width,
                                             row * conf.img_height))
                sample_images.save(conf.sample_location + "/" +
                                   "epoch_{}.jpg".format(i))
